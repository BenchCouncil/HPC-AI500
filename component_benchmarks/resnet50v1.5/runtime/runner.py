...
class Runner(object):
    ...
    def _get_run_config(mode, model_dir, use_xla, use_dali, gpu_memory_fraction, seed=None):
        ...
        config = tf.estimator.RunConfig(
            ...
            keep_checkpoint_max=10000, # default 5 
    ...
    def evaluate(
        ...
        try:
            ...
            # updated by xingw
            acc_js_path = os.path.join(self.run_hparams.model_dir, "acc.json")
            with open(acc_js_path, "w") as fd:
                import json
                json.dump({
                    "top1_accuracy": float(eval_results['top1_accuracy'] * 100),
                    "top5_accuracy": float(eval_results['top5_accuracy'] * 100),
                }, fd, indent=2)
                print("%s are saved!" % acc_js_path)
