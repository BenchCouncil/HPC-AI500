...
NUM_EPOCHS=1
SEED=10
OUTPUT_DIR=/data/imagenet-output/fp32-${NUM_EPOCHS}-SEED${SEED}-1GPU
...
#mpiexec --allow-run-as-root --bind-to none -map-by slot -np 32 --hostfile "./hostfile-4" -x NCCL_SOCKET_IFNAME=eth0 -x NCCL_DEBUG=info \
    python ${SOURCE_DIR}/main.py --mode=train --iter_unit=epoch --num_iter=${NUM_EPOCHS} --batch_size=128 --warmup_steps=100 --use_cosine_lr --label_smoothing 0.1 --lr_init=0.256 --lr_warmup_epochs=8 --momentum=0.875 --weight_decay=3.0517578125e-05 --data_dir=${DATA_DIR} --results_dir=${OUTPUT_DIR}/models --seed=${SEED} 
