...
CHECKPOINT_DIR=/data/imagenet-output/fp16-90-SEED10-2-8/
...
do
    ...
    cp -r ${ckpt_path%.*}* ${CHECKPOINT_DIR}/models/graph.pbtxt ${CHECKPOINT_DIR}/tmp/
    ...
    start=$(date +%s)
    python ${SOURCE_DIR}/main.py --mode=evaluate --data_dir=${DATA_DIR} --batch_size=256 --num_iter=1 --iter_unit=epoch --use_tf_amp --results_dir=${CHECKPOINT_DIR}/tmp
