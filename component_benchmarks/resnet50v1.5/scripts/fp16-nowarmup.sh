...
NUM_EPOCHS=90
SEED=10
OUTPUT_DIR=/data/imagenet-output/fp16-${NUM_EPOCHS}-SEED${SEED}-8-8-nowarmup
rm -rf ${OUTPUT_DIR}/*
mkdir -p ${OUTPUT_DIR}/models
cp ./fp16-nowarmup.sh ${OUTPUT_DIR}/
...
mpiexec --allow-run-as-root --bind-to none -map-by slot -np 64 --hostfile "./hostfile-8" -x NCCL_SOCKET_IFNAME=eth0 -x NCCL_DEBUG=info \
    python ${SOURCE_DIR}/main.py --mode=train --iter_unit=epoch --num_iter=${NUM_EPOCHS} --batch_size=256 --warmup_steps=0 --use_cosine_lr --label_smoothing 0.1 --lr_init=0.256 --lr_warmup_epochs=0 --momentum=0.875 --weight_decay=3.0517578125e-05 --use_tf_amp  --data_dir=${DATA_DIR} --results_dir=${OUTPUT_DIR}/models --seed=${SEED}
