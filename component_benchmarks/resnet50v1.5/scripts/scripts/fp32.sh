...
NUM_EPOCHS=50
OUTPUT_DIR=/data/imagenet-output/fp32-${NUM_EPOCHS}
mkdir -p ${OUTPUT_DIR}
rm -rf ${OUTPUT_DIR}/*
...
mpiexec --allow-run-as-root --bind-to socket -np 8 \
    python ${SOURCE_DIR}/main.py --mode=train --iter_unit=epoch --num_iter=${NUM_EPOCHS} --batch_size=128 --warmup_steps=100 --use_cosine_lr --label_smoothing 0.1 --lr_init=0.256 --lr_warmup_epochs=8 --momentum=0.875 --weight_decay=3.0517578125e-05 --data_dir=${DATA_DIR} --results_dir=${OUTPUT_DIR}
